...
describe("exam3", () => {
  test("頂いたcsv", async () => {
    expect(
      await getSTDOut(
        fork("src/exam3.js", ["-p", "test/test.csv"], { silent: true })
      )
    ).toBe(
      "サーバー10.20.30.1/16は過負荷状態です。期間は2020年10月19日13時31分24.000秒からです。\n"
    );
  });
...
describe("exam4", () => {
  test("頂いたcsv", async () => {
    expect(
      await getSTDOut(
        fork("src/exam4.js", ["-p", "test/test.csv"], { silent: true })
      )
    ).toBe(
      "サーバー10.20.30.1/16は過負荷状態です。期間は2020年10月19日13時31分24.000秒からです。\n"
    );
  ...
  test("test.csv", async () => {
    expect(
      await getSTDOut(
        fork("src/exam4.js", ["-p", "test/test4.csv"], { silent: true })
      )
    ).toBe(
      "サーバー10.20.30.1/16が復旧しました。故障期間は2020年1月2日3時4分6.000秒から2020年1月2日3時4分8.006秒までです。\n" +
        "サーバー10.20.30.2/16が復旧しました。故障期間は2020年1月2日3時4分6.000秒から2020年1月2日3時4分8.005秒までです。\n" +
        "サーバー10.20.30.3/16が復旧しました。故障期間は2020年1月2日3時4分6.000秒から2020年1月2日3時4分8.004秒までです。\n" +
        "サーバー20.20.30.4/16が復旧しました。故障期間は2020年1月2日3時4分6.000秒から2020年1月2日3時4分8.003秒までです。\n" +
        "サブネット10.20.0.0/16が故障しています。故障期間は2020年1月2日3時4分6.000秒から2020年1月2日3時4分8.004秒までです。\n" +
        "サブネット20.20.0.0/16が故障しています。故障期間は2020年1月2日3時4分6.000秒から2020年1月2日3時4分8.003秒までです。\n"
    );
  });
});
